...
for j in range(0, 1):
    ...
    param_grid = {
        ...
        'n_estimators': range(1, 100, 10),  # 决策树个数-随机森林特有参数

    }

    rfc = ensemble.RandomForestClassifier()
    GS = GridSearchCV(estimator=rfc, param_grid=param_grid).fit(xtrain, ytrain)
...
### 5.3 Compute AUC
meridian1 = model.predict_proba(xtest)
...
fpr, tpr, threshold = roc_curve(ytest_meridian, meridian1[:, 1])
roc_auc = auc(fpr, tpr)
print("AUC:", roc_auc)